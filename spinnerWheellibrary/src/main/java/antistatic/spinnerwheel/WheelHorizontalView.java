...
public class WheelHorizontalView extends AbstractWheelView {
	...
	public void setIsSelectorAlpaShow(boolean isShow){
		this.isSlectorAlpha = isShow;
	}
...
//		Log.e(LOG_TAG, " ---------- dumping finished ");
	...
	protected void drawItems(Canvas canvas) {
		...
		int left = (mCurrentItemIdx - mFirstItemIdx) * iw + (iw - getWidth())
				/ 2;
//		Log.d("itemindex", "left:"+String.valueOf(mCurrentItemIdx));
		c.translate(-left + mScrollingOffset, mItemsPadding);
//		Log.d("x","cur:"+String.valueOf(-left + mScrollingOffset));
		...
		if(isSlectorAlpha){
			if (mSelectionDivider != null) {
				...
				mSelectionDivider.setBounds(leftOfLeftDivider + 10, 0,
						rightOfLeftDivider - 10, getHeight());
				...
				mSelectionDivider.setBounds(leftOfRightDivider + 10, 0,
						rightOfRightDivider - 10, getHeight());
				mSelectionDivider.draw(cSeparators);
			}

			cSpin.drawRect(10, 0, w - 10, h, mSelectorWheelPaint);
		}
		

		if (getChartShowType() == ChartType.Curve) {
			Paint paint = new Paint();
			paint.setColor(Color.RED);
			cSpin.drawLine(w / 2.0f, h, w / 2.0f, 0, paint);
		}
		cSeparators.drawRect(10, 0, w - 10, h, mSeparatorsPaint);
